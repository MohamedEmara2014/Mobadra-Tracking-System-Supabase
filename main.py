    ...
    if st.session_state.role == "admin":
        ...
        if not full_df.empty:
            ...
            if 'updated_at' in full_df.columns:
                full_df['updated_at'] = pd.to_datetime(full_df['updated_at']).dt.tz_convert('Asia/Riyadh').dt.strftime('%Y-%m-%d %I:%M %p')
            ...
            for i, sec_name in enumerate(all_sections):
                with tabs[i]:
                    ...
                    if not sec_data.empty:
                        # --- دمج الوقت مع العنوان ---
                        last_up = sec_data['updated_at'].iloc[0] if 'updated_at' in sec_data.columns else "غير مسجل"
                        st.subheader(f"بيانات قسم {sec_name} (آخر تحديث: {last_up})")
                        ...
                        st.warning(f"لا توجد سجلات بيانات مرسلة من قسم {sec_name} حتى الآن.")
            ...
            with tabs[-1]:
                ...
                # زر تحميل الإكسيل للتقرير المجمع
